# Siniestro Fácil — Especificaciones SDD

## Fuente y alcance
Fuente funcional: entrevista de descubrimiento del caso de uso 2 — Seguros Horizonte: Siniestro Fácil.

**Regla de trazabilidad:** esta especificación no introduce políticas, umbrales, SLA, datos, integraciones ni decisiones no expresamente soportadas por la fuente. Todo punto no definido queda como pregunta abierta (Q-xxx).

## Documentos
- `01-vision-y-alcance.md`
- `02-historias-usuario.md`
- `03-requerimientos-funcionales.md`
- `04-requerimientos-no-funcionales.md`
- `05-criterios-aceptacion.md`
- `06-reglas-negocio.md`
- `07-modelo-dominio-y-flujos.md`
- `08-integraciones-y-eventos.md`
- `09-riesgos-y-preguntas-abiertas.md`
- `10-matriz-trazabilidad.md`
- `11-validaciones.md`

## Fuente
La especificación se deriva del archivo de entrevistas adjunto `02_seguros_siniestro_facil(1).md`. fileciteturn0file0

## Prototipo
Figma: **Siniestro Fácil - Prototipo Validaciones**. El prototipo es preliminar y representa únicamente comportamientos soportados por las entrevistas; no constituye una definición visual definitiva.
