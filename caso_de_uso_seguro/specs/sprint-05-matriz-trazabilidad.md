...
| S05-02 | Aprobar u observar presupuesto/cambio | US-012 | RF-015 | CA-010 | RN-006 | POST `/api/v1/siniestros/{id}/autorizaciones` / POST `/api/v1/presupuestos/{id}/observaciones` | `autorizacion` | En diseño | La decisión funcional está definida, pero falta definir el payload mínimo y la representación del responsable de la observación | Cerrar payload y representación del responsable antes de código |
...
| S05-06 | Pago/indemnización | US-019 | RF-018 | CA-013 | RN-005 | POST `/api/v1/siniestros/{id}/pagos` | `pago` | Definido | El pago/indemnización queda asociado a un siniestro autorizado, con trazabilidad del resultado y controles de idempotencia y duplicidad | Implementar como flujo económico del siniestro reutilizando la infraestructura existente |
| S05-07 | Control de pagos duplicados | US-019 | RF-018 | CA-014 | RN-012 | POST `/api/v1/siniestros/{id}/pagos` | `pago` + idempotencia técnica | Definido | Si ya existe una operación económica equivalente para el mismo siniestro y autorización, un nuevo pago debe tratarse como conflicto y no generar un segundo resultado económico | Implementar el control reutilizando la idempotencia existente; no inventar una restricción adicional sin decisión técnica formal |
| S05-08 | Idempotencia del pago | — | RF-018 | — | — | POST pagos + `Idempotency-Key` | `pago` + infraestructura de idempotencia | Definido; implementación y prueba pendientes | El comportamiento de idempotencia para pago está definido en el diseño técnico | Reutilizar la infraestructura existente e implementar prueba específica de pago |
...
| S05-11 | Cierre de expediente | — | RF-005 / RF-016 | CA-015 | — | POST `/api/v1/siniestros/{id}/transiciones` | `siniestro`, `siniestro_estado_historial`, `auditoria` | Definido | El cierre procede desde `LISTO_PARA_ENTREGA` o `INDEMNIZADO` cuando se completó la modalidad de resolución correspondiente, existe resultado económico registrado y la transición queda en el historial | Implementar únicamente las condiciones y transiciones ya definidas; no crear una entidad independiente de cierre |
| S05-12 | Historial/auditoría de operaciones | US-016 | RF-016 | CA-011 | RN-006 | Transiciones / autorizaciones / pagos | `siniestro_estado_historial`, `auditoria` | Definido; integración y pruebas pendientes | Las operaciones sensibles deben conservar trazabilidad y las transiciones deben registrar actor, fecha, evento y correlación; el historial funcional es append-only | Integrar auditoría con las operaciones de Sprint 5 y añadir pruebas de trazabilidad por operación |
